...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background-color: #f9fafb; padding: 30px; border-radius: 0 0 5px 5px;">
        <h2 style="color: #f59e0b;">Fonds escrow libérés manuellement</h2>

        <p>Bonjour {{ $admin->name }},</p>

        <p>Les fonds en escrow d'une expédition annulée ont été restitués manuellement à l'utilisateur concerné via la commande de correction.</p>

        <div style="background-color: white; border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; margin: 20px 0;">
            <p style="margin: 5px 0;"><strong>Utilisateur :</strong> {{ $shipment->sender?->name ?? '—' }}</p>
            <p style="margin: 5px 0;"><strong>Email :</strong> {{ $shipment->sender?->email ?? '—' }}</p>
            <p style="margin: 5px 0;"><strong>Montant restitué :</strong> <span style="color: #f59e0b; font-size: 20px; font-weight: bold;">{{ number_format($amount, 2) }} {{ $currency }}</span></p>
            <p style="margin: 5px 0;"><strong>Expédition :</strong> #{{ $shipment->id }}</p>
            <p style="margin: 5px 0;"><strong>Titre :</strong> {{ $shipment->title ?? '—' }}</p>
            <p style="margin: 5px 0;"><strong>Date de l'expédition :</strong> {{ $shipment->created_at?->format('d/m/Y') }}</p>
            <p style="margin: 5px 0;"><strong>Traité le :</strong> {{ now()->format('d/m/Y à H:i') }}</p>
        </div>
